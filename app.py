...
# تنسيق مظهر داكن واحترافي وإصلاح ألوان الخطوط لتصبح ساطعة وقابلة للقراءة
st.markdown("""
    <style>
    ...
    /* صندوق النتائج وإصلاح لون النصوص بداخله للأبيض الساطع */
    .result-box { background-color: #171a21; padding: 25px; border-radius: 8px; border-right: 5px solid #00ffcc; direction: rtl; margin-top: 20px; color: #ffffff; text-align: right; }
    .result-box p, .result-box span, .result-box b { color: #ffffff !important; font-size: 1.05rem; }
    
    /* صندوق بريميوم الذهبي لخطط الترقية والدفع */
    .premium-box { background-color: #221c11; padding: 20px; border-radius: 8px; border-right: 5px solid #ffad33; direction: rtl; margin-top: 15px; text-align: right; color: #ffffff; }
    .premium-box p, .premium-box h4 { color: #ffffff !important; }
    
    /* أزرار الترقية والدفع الفوري */
    .pay-button-gold { display: block; text-align: center; background-color: #ffad33; color: #0e1117 !important; font-weight: bold; padding: 10px; margin: 8px 0; border-radius: 5px; text-decoration: none; font-size: 1rem; }
    .pay-button-gold:hover { background-color: #e69519; }
    .pay-button-cyan { display: block; text-align: center; background-color: #00ffcc; color: #0e1117 !important; font-weight: bold; padding: 10px; margin: 8px 0; border-radius: 5px; text-decoration: none; font-size: 1rem; }
    .pay-button-cyan:hover { background-color: #00cca3; }
    
    /* صندوق شروط الاستخدام */
...
<b>بند 4: البصمة الرقمية والنسخ</b><br>
...
if accepted_tos:
    ...
    if st.button("🧮 بدء معالجة الرصد واستخراج التقرير النهائي"):
        ...
        # 🔒 الثوابت والمعادلات الحتمية المخفية في الخلفية
        ...
        if denominator == 0:
            ...
            # عرض النتائج النهائية بلون أبيض ناصع وواضح جداً للقراءة فوق الخلفية الداكنة
            st.markdown(f"""
            <div class="result-box">
                <h4 style='color: #00ffcc; margin-top:0; font-weight: bold;'>📋 نتائج تحليل المنظومة الديناميكية:</h4>
                ...
                <p style='font-size: 0.75rem; color: #00ffcc !important; direction: ltr; text-align: left; margin-bottom: 0;'>
                [DIGITAL-SIGNATURE: CHRONO-DYNAMICS-DETERMINISTIC-VERIFIED-2026]
                </p>
            </div>
            """, unsafe_allow_html=True)
            
            # صندوق غلق التفاصيل البرمجية مع روابط الدفع المباشرة والترقية الفورية
            st.markdown("""
            <div class="premium-box">
                ...
                الوصول إلى قِيَم <b>(معامل التشوه الهيكلي للوسط)</b> والمخططات البيانية والاشتقاقات الرياضية الكاملة لـ <b>(ثابت يوسف)</b> متاح الآن عبر الدفع والاشتراك الفوري:
                </p>
                
                <a href="https://www.paypal.com" target="_blank" class="pay-button-gold">💳 اشترك في الباقة المتقدمة للأفراد (Premium) - $29 / شهرياً</a>
                <a href="https://www.paypal.com" target="_blank" class="pay-button-cyan">🏛️ اشترك في الباقة الأكاديمية للمختبرات والمؤسسات - $149 / سنوياً</a>
                
                <p style='font-size: 0.85rem; color: #a3b3c2; margin-top: 15px; text-align: center;'>
                * بعد إتمام الدفع الفوري، سيتم إرسال مفتاح التفعيل التلقائي إلى بريدك الإلكتروني المسجل.
                </p>
            </div>
            """, unsafe_allow_html=True)
else:
