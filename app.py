...
# إعدادات الواجهة (مظهر داكن واحترافي)
st.set_page_config(
...
# تصميم مخصص متناسق ومريح للرصد الرقمي
st.markdown("""
    <style>
...
يقوم هذا المستشار بمعالجة بيانات رصد المواكب وحساب توزيع نسب المادة المظلمة الافتراضية كبديل ميكانيكي، معتمداً على آليات موازنة رياضية صارمة تمنع ظهور القيم غير المعرفة (كالصفر المطلق أو المالانهاية).
</div>
---
""", unsafe_allowed_html=True)
...
# معالجة وحساب النتائج بدقة عالية
if st.button("🧮 بدء معالجة الرصد واستخراج النتائج الدقيقة"):
    st.write("### 📊 المخرجات والنسب الحتمية المستقرة")
    
    # دمج الثابت لمنع حدوث الصفر المطلق في النظم البرمجية
    denominator = v_distortion + v_constant
    
    if denominator == 0:
        st.error("⚠️ فشل في النظام: المدخلات تؤدي إلى قيم حرجة غير معرفة.")
    else:
        # حساب المركب الكلي للمتجه
        result_vector = (v_force * v_velocity) / denominator
        
        # حساب النسبة المئوية للمادة المظلمة بناءً على التشوه ومقياس الثابت المتوازن
        ...
        <div class="result-box">
            ...
            <table style='width: 100%; border-collapse: collapse; color: #f0f2f6; direction: rtl;'>
                ...
                <tr style='border-bottom: 1px solid #2d3139;'>
                    <td style='padding: 10px 0;'><b>نسبة المادة المظلمة المكافئة المحسوبة:</b></td>
