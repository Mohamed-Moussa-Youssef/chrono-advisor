...
# إعدادات الواجهة الاحترافية المغلقة
st.set_page_config(
...
# تصميم المظهر الداكن وتنسيق خانات الإدخال الرقمية
st.markdown("""
    <style>
...
with col1:
    v_distortion = st.number_input("معامل التشوه الهيكلي للوسط (Distortion Coefficient):", value=0.5000, step=0.0100, format="%.4f")
    v_velocity = st.number_input("السرعة الرصدية للموكب/الجرام (v):", value=1.0000, step=0.1000, format="%.4f")

with col2:
    v_constant = st.number_input("عامل التوازن الثابت (Equilibrium Constant):", value=1e-11, step=1e-12, format="%.13f")
    v_force = st.number_input("القوة الديناميكية المؤثرة (F):", value=10.0000, step=1.0000, format="%.4f")
...
if st.button("🧮 بدء معالجة الرصد واستخراج النتائج الدقيقة"):
    ...
    if denominator == 0:
        ...
        # معادلة اشتقاقية دقيقة لربط التشوه بالكتلة الغائبة المفترضة
        ...
        <div class="result-box">
            ...
            <table style='width: 100%; border-collapse: collapse; color: #f0f2f6; direction: rtl;'>
                <tr style='border-bottom: 1px solid #2d3139;'>
                    <td style='padding: 10px 0;'><b>المركب الحتمي الكلي للمتجه الناتح:</b></td>
                    <td style='text-align: left; color: #00ffcc; font-size: 1.3rem;'><b>{result_vector:.6f}</b></td>
                </tr>
                <tr style='border-bottom: 1px solid #2d3139;'>
                    <td style='padding: 10px 0;'><b>نسبة المادة المظلمة المكافئة في الوسط:</b></td>
            ...
            * ملحوظة: الحسابات المخرجة تمثل محاكاة ديناميكية حتمية لحركة المتجهات الكتلية عند نقطة الاتزان الحرجة المستخلصة من بيانات التشوه الهيكلي.
            </p>
        </div>
        """, unsafe_allowed_html=True)
...
st.markdown("""
---
<p style='text-align: center; color: #4f5b66; font-size: 0.8rem;'>منصة الحسابات الفيزيائية الحتمية الرصدية © 2026 - معيار المعالجة الميكانيكية المستمرة</p>
""", unsafe_allowed_html=True)
